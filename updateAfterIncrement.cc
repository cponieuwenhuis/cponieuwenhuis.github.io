...
//Trapezoidal integration rule
        ...
        FirstPiolaStress[cellID][q]=P;
				...
				double traceE, traceT, vonmises, eqvstrain, slipfraction_conv_total;
        ...
        slipfraction_conv=slipfraction_iter;
			  slipfraction_conv_total = 0.0;
        double minValue = std::numeric_limits<double>::max();
				for (int entry = 0; entry < this->userInputs.numSlipSystems1; ++entry){
          slipfraction_conv_total += slipfraction_conv[cellID][q][entry];  // Add the value to total
          minValue = std::min(minValue, s_alpha_conv[cellID][q][entry]);
        }        
				...
				if (this->userInputs.writeOutput){
					...
					this->postprocessValues(cellID, q, 3, 0) = slipfraction_conv_total;
					this->postprocessValues(cellID, q, 4, 0) = minValue;
...
////////User Defined Variables for visualization outputs for cell_centers (outputoutputCellCenters_Var1 to outputoutputCellCenters_Var24)////////
	...
	if(Utilities::MPI::this_mpi_process(this->mpi_communicator)==0){
		...
		// Assuming this->userInputs.numberUserDefinedAverageOutput is an integer
    dir += std::string("stressstrain") + std::to_string(this->userInputs.currentSim) + ".txt";
