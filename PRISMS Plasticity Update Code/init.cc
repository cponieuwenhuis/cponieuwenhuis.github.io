...
void crystalPlasticity<dim>::init(unsigned int num_quad_points)
{
  ...
  //unsigned int start_idx = rank * num_local_cells;


  std::string line;
  unsigned int numElements, numGaussPoints, numSlipSystems;
  ...
  if(this->userInputs.enableMultiphase){
	...
	std::vector<std::vector<std::vector<double>>> s0_initDSP;	
  if(this->userInputs.enableDSP){
    s_alpha_conv.resize(num_local_cells,std::vector<Vector<double> >(num_quad_points,s0_init));
    s_alpha_iter.resize(num_local_cells,std::vector<Vector<double> >(num_quad_points,s0_init));
    
    std::ifstream DSPfile(this->userInputs.DSPfilename);

    //read data
    if (DSPfile.is_open()){
	
      std::getline(DSPfile, line);
      std::stringstream ss(line);
      ss >> numElements >> numGaussPoints >> numSlipSystems; 
    
      s0_initDSP.resize(num_local_cells, 
                  std::vector<std::vector<double>>(numGaussPoints, 
                                                   std::vector<double>(numSlipSystems, 0.0)));
		  //unsigned int start_idx = this->dofHandler.begin_active()->index();
      typename DoFHandler<dim>::active_cell_iterator cell = this->dofHandler.begin_active(), endc = this->dofHandler.end();
      unsigned int start_idx = dealii::numbers::invalid_unsigned_int;
      unsigned int idx = 0;
      for (; cell != endc; ++cell) {
        if (cell->is_locally_owned()) {
          
           start_idx = cell->index();
           
           //std::cout << "Start cell: " << start_idx << "for proc" << rank <<  '\n';
		   
		       for (unsigned int gauss = 0; gauss < num_quad_points; ++gauss) {
             if (!std::getline(DSPfile, line)) {
               std::cerr << "Error: Unexpected end of file while reading gauss point data." << std::endl;
               return; // Indicate error
             }

             std::stringstream gaussSS(line);
             // Loop over the slip systems for this Gauss point
             for (unsigned int slipSystem = 0; slipSystem < numSlipSystems; ++slipSystem) {
               if (!(gaussSS >> s0_initDSP[idx][gauss][slipSystem])) {
                 std::cerr << "Error: Invalid data for s0 at element " << start_idx
                           << ", Gauss point " << gauss << ", slip system " << slipSystem << std::endl;               

                 return; // Indicate error
               }
               
               s_alpha_conv[idx][gauss][slipSystem]=s0_initDSP[idx][gauss][slipSystem];
               s_alpha_iter[idx][gauss][slipSystem]=s0_initDSP[idx][gauss][slipSystem];
               //std::cout << "Start CRSS for Element: " << s_alpha_conv[idx][gauss][slipSystem] <<  '\n';
               
               if ((this->userInputs.enableUserMaterialModel)&&(this->userInputs.enableUserMaterialModel1)){
                 stateVar_conv[idx][gauss][slipSystem]=stateVar_init(slipSystem);
                 stateVar_iter[idx][gauss][slipSystem]=stateVar_init(slipSystem);
               }
           ...
           idx += 1;  
		      ...
		      for (unsigned int gauss = 0; gauss < num_quad_points; ++gauss) {
            if (!std::getline(DSPfile, line)) {
                std::cerr << "Error: Unexpected end of file while skipping lines." << std::endl;
                return; // Indicate error
            }
